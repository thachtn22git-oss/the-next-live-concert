...
export default function Navbar() {
  ...
  return <header className="site-header">
    ...
    {open && <nav id="mobile-navigation" className="mobile-nav site-container" aria-label="Điều hướng di động" onKeyDown={event => { if (event.key === 'Escape') setOpen(false); }}>{[...navigationItems, account, { path: '/my-tickets', label: 'Vé của tôi' }, ...(profile?.role === 'admin' ? [{ path: '/admin', label: 'Quản trị' }] : [])].map(item => <NavLink key={item.path} to={item.path} end={item.path === '/'} onClick={() => setOpen(false)}>{item.label}</NavLink>)}</nav>}
  </header>;
}
